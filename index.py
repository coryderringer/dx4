...
LengthOfData = 48 # switch to extinction phase after 48 trials
NumScenarios=1
...
class User(db.Model):
	...
	position2 =			db.IntegerProperty() # this is where the good/bad outcomes are (left or right)
	var1_Name =			db.StringProperty()
	var2_Names =		db.ListProperty(str)
	condition =			db.StringProperty() # pos/neg valence IS condition now, between subs
	storyCondition =	db.StringProperty() # story/monetary/combined
	drugColor =			db.StringProperty()
	diseaseName =		db.StringProperty()
...
class ScenarioData(db.Model):
	...
	# valence = 			db.StringProperty() # now between subs condition
	...
	position2 = 		db.IntegerProperty() # I the drug/no drug should always be drug on left...I'll talk to Ben about this
...
class AjaxHandler(webapp.RequestHandler):
	...
	def post(self):
		...
		var1_Left = self.session['var1_Name'][2*scenario]
		var1_Right = self.session['var1_Name'][2*scenario+1]
...
class AjaxMemoryHandler(webapp.RequestHandler):
	...
	def post(self):
		...
		condition = str(self.request.get('condition')) # pos/neg valence
	...
	def get(self):
		...
		try:
			...
			doRender(self, 'scenario.htm',
				...
				'condition':self.session['condition'], # pos/neg valence
				'var1_Name_Left': self.session['var1_Name'][2*scenario],
				'var1_Name_Right': self.session['var1_Name'][2*scenario+1],
				'var2_Names_Left': self.session['var2_Names'][2*scenario],
				'var2_Names_Right': self.session['var2_Names'][2*scenario+1],
				'disease': self.session['diseaseName'],
				'drugColor': self.session['drugColor'],
	...
	def post(self):
		...
		if(scenario >= NumScenarios): # refresh problem
			...
			obj = FinalJudgmentData(
				...
				drugColor = self.request.get('DrugColor'),
			...
			if scenario<=NumScenarios-1: #have more scenarios to go
				...
				doRender(self, 'prescenario.htm',
					{'disease':disease,
					'var1_Left_Name': var1_Name_Left,
					'var1_Right_Name': var1_Name_Right,
...
class DemographicsHandler(webapp.RequestHandler):
	...
	#race_num is a number corresponding to a single race AmerInd (1) - White(5)
			...
			self.session.__delitem__('diseaseName')
			self.session.__delitem__('drugColor')
	...
	def post(self):
		...
		if result.content=="0":
			...
			# If user is qualified (http://www.mturk-qualify.appspot.com returns 1)
				...
				# disease names
				self.session['diseaseName'] = 'Duastea'
				
				# drugColor = ['blue', 'green', 'orange', 'purple']

				# for this one we only need one drug name and color
				self.session['drugColor'] = random.choice(['lightblue', 'darkblue', 'orange', 'purple'])
				# drug names
				self.session['var1_Name'] = random.choice(['XF7', 'BT3', 'GS5', 'PR2'])
				...
				# condition is valence of drug outcomes. Note that "positive" isn't "rare-positive" anymore.
				# now in the "positive" condition, participants gain "points" when the drug is present. In the negative condition they lose "points."
				# so changing money to "points" is another thing to work on in the front end.
				if self.session['usernum'] % 2 == 0:
					self.session['condition'] = 'positive'
					
				else:
					self.session['condition'] = 'negative'

				self.session['runningBonuses'] = 100 # "points," not money
				...
				if self.session['position2'] == 0:
					self.session['var2_Names'] = ['BAD', 'GOOD']
				else:
					self.session['var2_Names'] = ['GOOD', 'BAD']


				# I don't think I need freq1 or freq2 for this experiment. Going to keep them in the back end just for now
				# and worry about it on the front end
				...
				# Data1_var1 is the drug: 0 is absent, 1 is present
				# if self.session['frequency1'] == 0:
				# 	a = [[1]*16, [0]*8, [1]*8, [0]*16, [1]*12, [0]*12, [1]*12, [0]*12] # 0 is drug absent, 1 is drug present
				# else:
				# 	a = [[1]*36, [0]*12] # 1 is common

				# var1: drug present(1) or absent(0)
				a1 = [[1]*16, [0]*8, [1]*8, [0]*16]
				a2 = [[1]*12, [0]*12, [1]*12, [0]*12] # 0 is drug absent, 1 is drug present
				
				a1 = [item for sublist in a1 for item in sublist] # flatten the lists
				a2 = [item for sublist in a2 for item in sublist] # flatten the lists
				

				logging.info("FLAT LIST A1: "+str(a1))
				# var2: good/bad outcome (1), or no change (0)
				b1 = [[1]*24, [0]*24] # 0 is outcome absent (no change), 1 is good/bad outcome
				
				b1 = [item for sublist in b1 for item in sublist] # flatten the list
				b2 = b1

				# random data order
				order1 = list(range(24))
				random.shuffle(order1)

				a = [] # originally data1 referred to scenario 1, but here it's the first half of the dataset
				b = []
				...
				# randomize second half:
				order2 = list(range(24))
				...
				# Data1_var1.append(Data2_var1)
				# Data1_var2.append(Data2_var2)

				self.session['v1_Data'] = a
				self.session['v2_Data'] = b

				logging.info('IS THIS STILL FLAT? '+str(a))
				# order of test questions
				...
				# already flat, shouldn't need this anymore
				...
				newuser = User(
					...
					v2_Data = b,
					testAttempts = [0,0],
					frequency1 = self.session['frequency1'], # think about freq1 and freq2 with this expt...this will be different! Both frequencies the same base rate now!
					frequency2 = self.session['frequency2'],
					position2 = self.session['position2'],
					var1_Name = self.session['var1_Name'],
					var2_Names = self.session['var2_Names'],
					condition = self.session['condition'], # pos/neg
					drugColor = self.session['drugColor'],
					diseaseName = self.session['diseaseName'],
					progress = 0);
